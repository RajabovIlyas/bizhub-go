...
func (s *EverydayWorkService) Of(employeeId primitive.ObjectID) *EverydayWorkServiceClient {
	...
	// s.mu.Lock()
	// defer s.mu.Unlock() // eger bu yerde lock etsek, onda Of(employeeID) ishleyan yerlerini lock() etmeli dal!
	...
	if !ok {
		client := &EverydayWorkServiceClient{
			...
			queue:      make(chan *EverydayWorkTask, 100),
		}
		go client.run()
...
func (s *EverydayWorkService) getOnlyDate(date time.Time) time.Time {
	y, m, d := date.Date()
	nDate := time.Date(y, m, d, 0, 0, 0, 0, time.UTC)
	return nDate
}
...
func (s *EverydayWorkService) createDailyDocuments() {
	...
	for _, employeeId := range employees {
		if s.employees[employeeId.Hex()].date.Equal(date) {
			continue
		}
